...
st.set_page_config(
    ...
    page_icon="icon.png",
    layout="wide",
    initial_sidebar_state="collapsed"   # mulai collapsed, hamburger akan toggle
)
...
# ======================
# HELPERS WARNA SIDEBAR
# Sidebar navy  (#01023B) → untuk Admin
# Sidebar dark  (#2D303E) → untuk halaman role/login/user/dev
# ======================
def _sidebar_bg():
    return "#01023B" if (
        st.session_state.login and st.session_state.role == "admin"
    ) else "#2D303E"
...
<style>
/* ── Global ── */
[data-testid="stAppViewContainer"] {{ background: #F3FDF9; }}
...
/* ── Hamburger button (injected via markdown, floats top-left) ── */
.hamburger-wrap {{
    position: relative;
    z-index: 999;
...
.stWarning  {{ border-radius: 10px !important; font-size: 13px !important; }}
.stError    {{ border-radius: 10px !important; font-size: 13px !important; }}
.stInfo {{
    background: #E6FDF5 !important;
    border: 1px solid #D1FAE5 !important;
    border-radius: 10px !important;
...
/* ── Checkbox ── */
.stCheckbox > label {{ font-size: 13px !important; color: #01023B !important; }}

/* ── Radio (sidebar) ── */
.stRadio > label {{ color: rgba(255,255,255,0.55) !important; font-size: 13px !important; }}
...
def generate_pdf(df, title="Laporan KasKita"):
    ...
    elements += [t, Spacer(1,16), Paragraph("© KasKita 2026", styles["Normal"])]
    doc.build(elements)
    buffer.seek(0)
    return buffer
...
def sidebar_sep():
    st.markdown('<div style="height:1px;background:rgba(255,255,255,0.07);margin:6px 0;"></div>',
                unsafe_allow_html=True)
...
def page_header(title, subtitle=None):
    st.markdown(f"<h2 style='font-size:20px;font-weight:500;color:#01023B;margin:0;'>{title}</h2>",
                unsafe_allow_html=True)
    if subtitle:
        st.markdown(f"<p style='font-size:12px;color:#6B7280;margin:3px 0 1rem;'>{subtitle}</p>",
                    unsafe_allow_html=True)
    else:
        st.markdown("<div style='margin-bottom:1rem;'></div>", unsafe_allow_html=True)

def hdivider():
    st.markdown('<hr style="border:0;border-top:1px solid #D1FAE5;margin:0.75rem 0;">',
                unsafe_allow_html=True)

# ======================
# HAMBURGER BUTTON
# Hanya ditampilkan di halaman user / role / login / dev
# Admin pakai sidebar fixed seperti sebelumnya
# ======================
def hamburger_button():
    """Tombol hamburger di area konten utama untuk toggle sidebar."""
    icon  = "✕  Tutup menu" if st.session_state.sidebar_open else "☰  Menu"
    color = "#09F289"
    if st.button(icon, key="hamburger_toggle"):
        st.session_state.sidebar_open = not st.session_state.sidebar_open
        # Streamlit tidak bisa programmatically expand sidebar,
        # tapi kita bisa simulate dengan set_page_config tidak bisa dipanggil ulang.
        # Solusi: kita render sidebar content hanya jika sidebar_open == True.
        st.rerun()
...
if not st.session_state.login and st.session_state.page == "role":
    ...
    # Sidebar konten (muncul jika sidebar_open)
    with st.sidebar:
        render_logo_sidebar()
        st.markdown("""
        <div style="font-size:13px;color:rgba(255,255,255,0.5);padding:8px 4px;">
            Pilih peran di halaman utama untuk masuk ke aplikasi.
        </div>
        """, unsafe_allow_html=True)
        sidebar_sep()
        with st.expander("🔧  Login Developer"):
            du = st.text_input("Username", key="dev_u_sb")
            dp = st.text_input("Password", type="password", key="dev_p_sb")
            if st.button("Login Developer", key="btn_dev_sb"):
...
# ======================
    ...
    with st.sidebar:
        render_logo_sidebar()
        st.markdown("""
        <div style="font-size:12px;color:rgba(255,255,255,0.4);padding:4px 4px 12px;">
            Masukkan detail akun untuk login sebagai Admin.
        </div>
        """, unsafe_allow_html=True)
        sidebar_sep()
        if st.button("← Kembali pilih role", use_container_width=True, key="back_sb"):
...
# ======================
    ...
    if st.session_state.role == "user":
        ...
        # Sidebar (konten muncul selalu, user toggle via hamburger)
        with st.sidebar:
            render_logo_sidebar()
        ...
        if df_all.empty:
            ...
            with tab1:
                ...
                st.download_button(
                    ...
                    f"kas_{fk}_{fj}.pdf", mime="application/pdf"
                )
            with tab2:
    ...
    # ==================== DEVELOPER ====================
        ...
        if menu_dev == "Akun Admin":
            st.markdown("<div style='font-size:14px;font-weight:500;color:#01023B;margin-bottom:.75rem;'>Daftar akun admin</div>", unsafe_allow_html=True)
            if df_admin.empty:
                st.info("Belum ada akun admin.")
            else:
                st.dataframe(df_admin, use_container_width=True, hide_index=True)
                st.download_button("⬇  Download PDF akun", generate_pdf(df_admin,"Data Akun Admin"), "akun_admin.pdf", mime="application/pdf")
                hdivider()
            ...
            st.markdown("<div style='font-size:14px;font-weight:500;color:#01023B;margin-bottom:.75rem;'>Semua data kas</div>", unsafe_allow_html=True)
            if df_kas.empty:
                st.info("Belum ada data kas.")
            else:
                st.dataframe(df_kas, use_container_width=True, hide_index=True)
                st.download_button("⬇  Download PDF kas", generate_pdf(df_kas,"Semua Data Kas"), "kas_all.pdf", mime="application/pdf")
    ...
    # ==================== ADMIN ====================
        ...
        # Admin: sidebar selalu terbuka (expanded), tanpa hamburger
        with st.sidebar:
            render_logo_sidebar()
        ...
        # ==================== PENGELUARAN ====================
            ...
            df_keluar = pd.read_sql("SELECT * FROM pengeluaran WHERE kelas=? AND jurusan=?", conn, params=(kls,jrs))
            df_masuk  = pd.read_sql("SELECT nominal FROM kas WHERE kelas=? AND jurusan=?", conn, params=(kls,jrs))
            ...
            if df_keluar.empty:
                ...
                st.download_button("⬇  Download PDF pengeluaran",
        ...
        # ==================== HAPUS DATA ====================
            ...
            with c3:
                ...
                if st.button("Hapus semua", key="btn_hall", type="primary"):
                    if konfirmasi:
                        cursor.execute("DELETE FROM kas WHERE kelas=? AND jurusan=?", (kls,jrs))
                        conn.commit()
