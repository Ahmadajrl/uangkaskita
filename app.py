...
:root {
    --bg-primary: #080e1d; --bg-card: rgba(255,255,255,0.04); --bg-card-hover: rgba(255,255,255,0.07);
    --border: rgba(255,255,255,0.08); --accent: #3b82f6; --accent-glow: rgba(59,130,246,0.25);
    --accent2: #06b6d4; --green: #10b981; --red: #f43f5e; --gold: #f59e0b;
    --text-primary: #f1f5f9; --text-muted: #64748b; --radius: 16px; --radius-sm: 10px;
}
...
def validasi_kas(nama, kelas, jurusan, nominal):
    ...
    if len(nama) < 3: return "Nama terlalu pendek"
        ...
        if st.button("Login"):
            if not user or not pw:
                ...
                if not df.empty:
                    ...
                    data = df[(df["username"] == user) & (df["password"] == hash_password(pw))]
                    if not data.empty:
    ...
    with tab2:
        ...
        if st.button("Daftar"):
            if not user or not pw:
                ...
                api_post("admin", {"username": user.strip(), "password": hash_password(pw)})
                st.success("Akun berhasil dibuat")

# ================= MAIN =================
else:
    # ✅ MENU PENGELUARAN & LOGOUT (DITEMPATKAN DI SIDEBAR)
    with st.sidebar:
        st.markdown(f"""
        <div class="sidebar-user">
            <div class="user-avatar">{st.session_state.user[0].upper()}</div>
            <div class="user-info">
                <div class="user-label">AKTIF</div>
        ...
        if st.button("📊 Dashboard", use_container_width=True, key="btn_dashboard"):
            ...
            st.rerun()
        if st.button("💸 Pengeluaran", use_container_width=True, key="btn_pengeluaran"):
        ...
        if st.button("Simpan Pengeluaran"):
            if not ket.strip():
                ...
                api_post("pengeluaran", {
                    "tanggal": str(tgl), "keterangan": ket.strip(),
                    "nominal": int(nom), "owner": st.session_state.user
                })
